...
void dummy_activate_down_suggestion(void)
{
...
static int first=1;
	...
	{	first=1;
		//printf("\nwifiInteraction tar hot aahe re bhau\n the value of wifInteraction is %d", wifiInteraction);	
		wifiInteraction=1;
		sprintf(wifi.activity,"ON");
		//printf("\nthe value of wifiInteraction and is %d and wifi's counter is %d\n", wifiInteraction, wifiCount);

		if (check_wireless_powersave()) 
		{
			sprintf(wifi.remark,"WIFI PUT TO POWER SAVE MODE");
			sprintf(wifi.activity,"POWER SAVE");
			//printf("\npower saving che suggestion dene yogya !!\n");
